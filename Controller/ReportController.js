...
export const PrepareTaxCode = async (ProvinceId, FromDate, ToDate) => {
  ...
  var data = await CustomerModel.findAll({
    attributes: [
      sequelize.fn("DISTINCT", sequelize.col("CustomerModel.Id")),
      "Taxcode",
      "IsKeepInvocie",
    ],
    include: [
...
export const ExportHaNoiData = async (Customers, FromDate, ToDate, Type) => {
  Customers.forEach(async (customer) => {
    ...
    const NoticeIds = [
      ...new Set(invoices.map((i) => parseInt(i.NoticeIssuedId))),
    ];
    for (var i = 0; i < NoticeIds.length; i++) {
      var InvoicesByNotice = invoices.filter(
        (e) => e.NoticeissuedId == NoticeIds[i]
      );
      if (InvoicesByNotice.length < 1) continue;
      InvoicesByNotice.sort((a, b) =>
      ...
      if (customer.IsKeepInvocie && parseInt(customer.IsKeepInvocie) == 1) {
        const lastSignedInvoiceInList = InvoicesByNotice.filter(
          (e) => e.Status !== 1
        ).reduce((prev, current) =>
          prev.InvoiceNumber > current.InvoiceNumber ? prev : current
        );
        if (!lastSignedInvoiceInList) continue;
        const ListNotSigned = InvoicesByNotice.filter(
          (e) =>
            e.InvoiceNumber < lastSignedInvoiceInList.InvoiceNumber &&
            e.Status === 1
        );
        if (ListNotSigned && ListNotSigned.length > 0) {
          ...
          ListNotSigned.forEach(async (e) => {
            ...
            //Update result
            InvoicesByNotice[
              InvoicesByNotice.findIndex((e) => e.Id == invoice.Id)
            ] = invoice;
          });
      ...
      InvoicesByNotice.forEach((invoice) => {
        InvoicesByNotice[
          InvoicesByNotice.findIndex((e) => e.Id == invoice.Id)
        ] = new ReportTaxMaster(invoice);
        invoices[invoices.findIndex((e) => e.Id == invoice.Id)] =
          new ReportTaxMaster(invoice);
      });
      //List Adjusted & Replaced Invoices
      var listProcess = InvoicesByNotice.filter(
        (e) => e.Status === 3 || e.Status === 6
      );
      if (listProcess.length > 0) {
        var processedIds = InvoicesByNotice.map((e) => parseInt(e.Id));
        processedIds.push(
          InvoicesByNotice.map((e) => parseInt(e.IvoiceAdjustedId))
        );
        processedIds.push(
          InvoicesByNotice.map((e) => parseInt(e.IvoiceChangeId))
        );

        var MissingInvoiceIds = processedIds.filter(
          (x) => !invoiceIds.includes(x)
        );
        var MissingInvoices = await GetInvoicesByIds(
          MissingInvoiceIds,
          customer.Id
        );
        ...
        if (listAdjusted.length > 0) {
          ...
          listAdjustedLinks.forEach((adj) => {
            ...
            if (!process) {
              var newprocess = MissingInvoices.find(
                (e) => e.Id == adj.AdjustedInvoiceId
              );
            ...
            invoices[invoices.findIndex((i) => i.Id == processed.Id)] =
              processed;
          });
        }
...
const MapToTaxHaNoiData = (invoices) => {
  ...
  invoices.forEach((invoice) => {
    if (invoice.Status == 5) invoice.Status = 1;
      ...
      invoice.Status == 6 &&
      invoice.IsChange != null &&
      invoice.IsChange == 1
    )
      invoice.Status = 2;
    else if (
      invoice.Status == 3 &&
      invoice.IsAdjusted != null &&
    ...
    if (invoice.IsConvert != null && invoice.IsConvert == 1)
      invoice.IsConvert = 1;
    else invoice.IsConvert = 0;
    if (invoice.MoneyCode.trim().toUpperCase() == "VND")
    ...
    if (invoice.SignXMLBuy && invoice.SignXMLBuy.toString().trim() != "") {
      ...
      if (nodes.length == 2) {
        ...
        invoice.SellerCertificate = invoice.SignXML.substring(
          invoice.SignXML.lastIndexOf("<X509Certificate>") + 1,
          str.lastIndexOf("</X509Certificate>")
        );
      ...
      var NearistSigned = invoices.find(
        (e) =>
          (e.InvoiceNumber > invoice.InvoiceNumber ||
            e.InvoiceNumber < invoice.InvoiceNumber) &&
          e.Status != 1
      );
      invoice.SellerCertificate = NearistSigned.SignXML.substring(
        NearistSigned.SignXML.lastIndexOf("<X509Certificate>") + 1,
        str.lastIndexOf("</X509Certificate>")
      );
      invoice.BuyerCertificate = "";
    }
...
const GetTaxReportDetail = async (invoices, invoiceIds) => {
  ...
  function loadDetail() {
    return new Promise((resolve, reject) => {
      var detailpart = InvoiceDetailModel.findAll({
        where: {
          IvoiceId: invoiceIds,
        },
      });
  ...
  loadDetail().then((details) => {
    for (let x = 0; x < invoices.length; x++) {
      ...
      for (let y = 0; y < invoiceDetails.length; y++) {
        invoiceDetails[y].Tax =
          invoiceDetails[y].Tax == 0 && invoice.Tax != 0
            ? invoice.Tax
            : invoiceDetails[y].Tax;
        ...
        invoiceDetails[y].Tax =
          invoiceDetails[y].Tax == -1 ? 0 : invoiceDetails[y].Tax;
        invoiceDetails[y].MoneyTax = invoiceDetails[y].MoneyTax
          ? invoiceDetails[y].MoneyTax
          : (invoiceDetails[y].TotalMoney * invoiceDetails[y].Tax) / 100;
        if (
          invoiceDetails[y].Discount &&
          y > 0 &&
          invoiceDetails[y].IvoiceId == invoiceDetails[y - 1].IvoiceId
        ) {
          ...
          invoiceDetails[y - 1].TotalMoney =
            invoiceDetails[y - 1].TotalMoney - invoiceDetails[y].DiscountMoney;
          invoiceDetails[y - 1].MoneyTax =
            (invoiceDetails[y - 1].TotalMoney * invoiceDetails[y - 1].Tax) /
            100;
          invoiceDetails[y - 1].TotalMoney =
            invoiceDetails[y - 1].TotalMoney + invoiceDetails[y - 1].MoneyTax;
        } else if (
          invoiceDetails[y].Discount &&
          y > 0 &&
          invoiceDetails[y].IvoiceId == invoiceDetails[y + 1].IvoiceId
...
const ExportExcel = async (taxcode, invoices, details, type) => {
  const d = new Date();
  const dirpath = `/ExportExcel/${d.getDate().toString().padStart(2, "0")}-${(
    d.getMonth() + 1
  )
    .toString()
  ...
  if (type == 1) {
    //#region Hanle Files
    let MasterWorkbook;
    MasterWorkbook = await new Excel.Workbook().xlsx.readFile(
      path.join(__dirname + "/public/TaxReportMasterTemplate.xlsx")
    );
    ...
    let DetailWorkbook;
    DetailWorkbook = await new Excel.Workbook().xlsx.readFile(
      path.join(__dirname + "/public/TaxReportDetailTemplate.xlsx")
    );
    ...
    for (let i = 0; i < invoices.length; i++) {
      ...
      else if (d.TemptCode.Contains("XKNB"))
        loaihoadon = "Hóa đơn xuất kho kiêm vận chuyển nội bộ";
    ...
    MasterWorkbook.xlsx.writeFile(
      path.join(__dirname + `/ExportExcel/${dirpath}/TaxReportMaster.xlsx`)
    );
    //#endregion
    ...
    //WriteFile
    DetailWorkbook.xlsx.writeFile(
      path.join(__dirname + `/ExportExcel/${dirpath}/TaxReportDetail.xlsx`)
    );
  } else {
    //#region Hanle Files
    let workbook = await new Excel.Workbook().xlsx.readFile(
      path.join(__dirname + "/public/TaxReportTemplate.xlsx")
    );
    ...
    workbook.xlsx.writeFile(
      path.join(__dirname + `/ExportExcel/${dirpath}/${taxcode}.xlsx`)
    );
  }
};
